...
def load_results():
    ...
    construct_sequence = "".join(
        line.strip()
        for line in (DATA / "my_construct.fasta").read_text().splitlines()
        if not line.startswith(">")
    )
    return {
        "construct": {"name": "Course fold: KRAS G12D residues 1–169", "pdb": "/structures/course-fold.pdb", "plddt": fold["plddt"], "pae": fold["pae"], "mean_plddt": fold["mean_plddt"], "ptm": fold["ptm"], "length": 169, "residue": "D12", "sequence": construct_sequence},
...
STRUCTURES = {
    "course-fold.pdb": RESULTS / "actual_construct_fold.pdb",
    "alphafold.pdb": RESULTS / "alphafold_reference.pdb",
    "alphafold.cif": DATA / "KRAS_alphafold_model.cif",
}
...
async function load(which){current=which;selected=null;let d=data[which];document.getElementById('viewer').innerHTML='';const viewerElement=document.getElementById('viewer');viewer=$3Dmol.createViewer(viewerElement,{backgroundColor:'white'});viewer.resize();let pdb=await (await fetch(d.pdb)).text();viewer.addModel(pdb,'pdb');resetStyles();viewer.setClickable({},true,(atom)=>select(atom.resi));viewer.zoomTo();viewer.render();drawSequence();draw(d)}

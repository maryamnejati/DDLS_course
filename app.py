...
async function load(which){current=which;selected=null;let d=data[which];document.getElementById('viewer').innerHTML='';viewer=$3Dmol.createViewer(document.getElementById('viewer'),{backgroundColor:'white'});viewer.resize();let pdb=await (await fetch(d.pdb)).text();viewer.addModel(pdb,'pdb');resetStyles();viewer.setClickable({},true,(atom)=>select(atom.resi));viewer.zoomTo();viewer.render();drawSequence();draw(d)}
